	...
	$( document ).ready(function() {
		...
		var datatable = $('.m_datatable').mDatatable({
			...
			columns: [{
                ...
                title: "Alunos Não Evadidos",
            },{
				field: "use_classify",
				title: "Classificar",
                template: function (row) {
                    if(row.use_classify == 1){
                        return '<label class="m-checkbox" style="margin-bottom: 15px;"><input onchange="alterState(\'course/use_classify\','+ row.id + ')" type="checkbox" checked="checked"><span></span></label>';
                    }else{
                        return '<label class="m-checkbox" style="margin-bottom: 15px;"><input onchange="alterState(\'course/use_classify\','+ row.id + ')" type="checkbox" ><span></span></label>';
                    }
                }
			}]
		});
